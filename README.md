# tiny-rmr-1-2.05 —— RMR 1:2.05 微型反射式红点瞄准镜（固件 + 设计 + 调试器）

> 完整说明书（固件特性/算法/按键操作/NVM 配置/编译烧录/调试）：**[docs/RMR_说明书.md](docs/RMR_说明书.md)**
...
- **修复 4（自动降亮度/关机计时不准，V4.3.4 补丁 2）**：`g_tick_ms % 1000 == 0` 在 delay_cycles 忙等构建（DIRECT/BATT/DBG）主循环 >1ms 时会漏整秒边界（实测计数率仅 0.37/s，40min 会拖到 ~108min）；改为 tick 差值计数（`g_tick_ms - last_sec_tick >= 1000`），实测 1/s 精确，40s 测试参数下 dim 在 ~41s、关机在 ~60s 触发
- **新增 STANDBY 确认计数（mailbox 0xC0）**：`standby_entry_cnt` 每次 OFF 会话第一次真正进入 STANDBY1 前 +1（SRAM 在 STANDBY1 下保留，唤醒后可读）；RMRDebugger 新增 "stby_cnt" 显示 + 修复 sys_clk 显示（新固件报 24000，旧逻辑只认 32000/4000 会显示 "-"）。实测 cmd=7 关机后 stby_cnt 0→1、PMODECFG=1、SWD 断开
...
- **产物**：六变体 hex 全部干净重建并逐个体检版本串（DIRECT/BATT/ECO_D/ECO_B/DBG/DBGL），0 错误 0 警告
## 更新记录：2026-08-14，修复“只能切 ALS 无法切回 MAN”
- **根因**：RUN 态双键按住 1.5s 即熄灯进入 OFF，继续按到 5s 时状态已在 OFF，而 OFF 分支的 5s 处理**无条件强制 ALS 开机**——导致从 ALS 永远切不回 MAN（熄灯→5s→又 ALS）
- **修复 1（模式互切）**：`empty_mspm0c1104.c` OFF 态 5s 事件区分来源——来自 RUN 熄灯路径（`g_off_pending`）时**切换 ALS<->MAN 并重新亮灯**；OFF 直接长按/开机失败重试仍为 ALS 快捷开机；NVM 写入改为按位设置支持清零
- **修复 2（ALS 锁定可退出）**：`sys_mode.c` 传感器故障锁定（lockout）后，**ALS 模式仍可切回手动**（原逻辑无条件拒绝所有切换导致卡死）；手动切 ALS 仍被拒以防闪烁循环
- **产物**：六变体 hex 全部重建（DIRECT/BATT/ECO_D/ECO_B/DBG/DBGL）+ 发布镜像 firmware/RMR.hex，0 错误 0 警告

## 更新记录：2026-08-14，无操作超时调整 + 新增完整说明书
- **超时调整**：无操作调暗由 60 分钟改为 **40 分钟**（`TIME_AUTO_DIM_S` 3600→2400），调暗后再 **10 分钟**无操作自动关机（`TIME_AUTO_SHUTDOWN_S=600` 不变）
- **说明书**：新增 [docs/RMR_说明书.md](docs/RMR_说明书.md)（固件特性确认/电压补偿算法/按键操作/状态机/NVM 配置/调试接口/编译烧录）
- **六变体 hex 全部重建**：DIRECT / BATT / ECO_D / ECO_B / DBG / DBGL（含发布镜像 firmware/RMR.hex），源码与 CCS 工作区同步

## 更新记录：2026-08-13，双键关机确认机制（灭灯≠standby）+ RMRDebugger 烧录修复
- **状态**：RUN 态双键满 1.5s 立即熄灯，继续按住到 5s 转 ALS 亮灯
- **机制**：1.5s 熄灯后进入 SYS_OFF，但不再立刻进 STANDBY0（防止 WCOMP/GPIO 唤醒源误触发导致“松开灯又亮”）
- **关机确认**：新增“灭灯≠standby”逻辑，`hal_keys.c` 新增 `EVT_BOTH_RELEASE_1_5S`：1.5s 熄灯后若未满 5s 松开 → 保持关机并进 standby；`empty_mspm0c1104.c` 新增 `g_off_pending` 门控，1.5s 后仅置 pending，确认松开才进 STANDBY0；按住满 5s → 清 pending + 闪烁 + ALS 亮灯
- **RMRDebugger 烧录修复**：`debugworkers.cpp` executeFlash 在 program 前加 `halt` + `flash probe 0`，若 target 无法 halt / FACTORYREGION flash size=0 → 提示 "Target missing or protected"；烧录后清 SRAM 开机门控 `g_off_intent`=0x20000299 / `g_por_magic`=0x2000029C，模拟冷启动自动亮灯（避免“烧录后 OFF 不亮灯”）
- **产物**：DEBUG_LP_BUILD / DEBUG_BUILD / 默认三配置 0 错误 0 警告；`firmware/RMR/hex/RMR_DBGL.hex`（V4.3.3_DBGL）已更新，RMRDebugger 烧录验证：1.5s 熄灯松开后保持 standby，5s 转 ALS + 亮灯
