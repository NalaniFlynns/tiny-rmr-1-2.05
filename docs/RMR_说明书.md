...
| 双键同时按住 5s | RUN / TEST | 切换 ALS <-> 手动，双闪提示；ALS 故障锁定时拒绝切回（双闪提示） |
| 双键同时按住 1.5s | RUN / LVP_CRIT | 立即熄灯进入 OFF（不等松开） |
| 双键按住 1.5s 后松开（未满 5s） | OFF | 确认关机，进入 STANDBY0 深睡 |
| 双键按住满 5s | OFF（熄灯后继续按） | 双闪提示 + 以 ALS 模式重新开机（测压达标才启动） |
...
说明：1.5s 熄灯不等于立即进深睡——先置 g_off_pending，确认松开且未满 5s 才进 STANDBY0；满 5s 则清 pending 并以 ALS 开机。这是“松开灯又亮”根因修复后的最终行为。
...
- 连续 3 次“故障->自恢复”后锁定 ALS（拒绝切回 ALS，避免传感器永久损坏时的闪烁循环）
## 9. 电源管理与保护
